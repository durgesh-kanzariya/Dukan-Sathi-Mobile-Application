...
class ShopService extends GetxService {
  ...
  // --- MODIFIED: Update Shop Image with Cloudinary ---
  Future<void> updateShopImage(XFile imageFile) async {
    if (currentShop.value == null) {
      Get.snackbar('Error', 'No shop found');
      return;
    ...
    try {
      ...
      // 3. Skip Deleting old image (unsafe/complex without backend)
      ...
      print('Error updating shop image: $e');
      Get.snackbar('Error', 'Failed to update image. Check internet.');
    } finally {
      isUploadingImage.value = false;
    }
  }

  // --- NEW: Helper function for Cloudinary ---
  Future<String> _uploadShopImage(XFile imageFile) async {
    try {
      CloudinaryResponse response = await cloudinary.uploadFile(
        CloudinaryFile.fromFile(
          ...
          folder: 'shop_images', // <--- Specific folder for shops
        ),
      );
